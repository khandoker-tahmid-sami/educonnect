...
const CoursesPage = async () => {
  ...
  const courseResonse = await fetch(`${base}/api/courses/get`, {
    cache: "no-store",
  });

  if (!courseResonse.ok) throw new Error("Failed to load course");

  const { data: courses } = await courseResonse.json();
  //   console.log(courses);
  return (
    <section
      id="courses"
